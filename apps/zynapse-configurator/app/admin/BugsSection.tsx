  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {bugs.length === 0 ? (
        ...
        <>
        {/* ── TELEFON (sub 768px): un raport = o cartela ───────────────────────────────────────
            Capul e cine a raportat si cand; apoi statusul; apoi RAPORTUL, care e tot rostul
            ecranului si care intr-un tabel de 5 coloane pe 360px ajungea o coloana de 2 cuvinte. */}
        <div className="divide-y divide-slate-100 md:hidden">
          {bugs.map(b => (
            <div key={b.id} className="px-5 py-4">
              <div className="flex items-start justify-between gap-3">
                <p className="min-w-0 break-all text-sm font-medium text-slate-800">{b.email}</p>
                {b.z_coins_awarded > 0 && (
                  <span className="shrink-0 font-semibold tabular-nums text-emerald-700">+{b.z_coins_awarded}</span>
                )}
              </div>
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm text-slate-700">{b.content}</p>
              {b.z_coins_awarded === 0 && (
                <div className="mt-3 flex gap-2">
                  <input type="number" min={1} max={10000} placeholder="Z-coins" inputMode="numeric"
                    value={amounts[b.id] ?? ""}
                    onChange={e => setAmounts(a => ({ ...a, [b.id]: e.target.value }))}
                    className="min-h-[44px] w-28 rounded border border-slate-300 px-3 text-right text-sm" />
                  <button type="button" onClick={() => void award(b.id)}
                    disabled={busy === b.id || !(Math.floor(Number(amounts[b.id] || 0)) > 0)}
                    className="min-h-[44px] flex-1 rounded bg-emerald-600 px-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300">
                    {busy === b.id ? "..." : "Acordă"}
                  </button>
                </div>
              )}
