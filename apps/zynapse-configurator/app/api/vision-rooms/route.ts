...
7. SUPRAFETE (din cartuș / tabel bilanț-indici) — extrage numeric dacă există: suprafața construită (Sc) -> surfaces.construita_mp; suprafața utilă (Su) -> surfaces.utila_mp; suprafața construită desfășurată (Scd) -> surfaces.desfasurata_mp. Caută texte gen "Suprafata construita = NNN mp" sau "Sc = NNN". Extrage DOAR numărul (ex. 245.73); negăsit -> null. NU inventa, NU estima. Dacă sunt MAI MULTE construcții (C1, C2, C3), ia construcția PRINCIPALĂ (C1 / casa de locuit), NU rândul TOTAL/TOTALA; pune în surfaces.note ce construcție ai ales (ex. "C1 CASA"). IMPORTANT desfășurată vs total: surfaces.desfasurata_mp = suprafața construită DESFĂȘURATĂ = suma NIVELURILOR aceleiași case (demisol+parter+mansardă); la case cu mai multe niveluri (D+P+M) ia TOTALUL desfășurat (ex. 240 = 80+80+80), NU un singur nivel. NU confunda cu TOTALA mai multor clădiri (C1+C2+C3) — aia NU se ia, rămâne doar construcția principală C1. Regulă cheie: niveluri ale ACELEIAȘI case se SUMEAZĂ în desfasurata; construcții DIFERITE -> doar principala. Pentru utilă ia valoarea totală (utila totală) dacă există. Bilanțul complet (niveluri + desfășurată) e mereu pe planul de PARTER.
