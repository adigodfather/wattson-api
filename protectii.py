...
e neobisnuit — curba B sare la 3..5x In, iar pornirea unui motor trece de-acolo. O tratez ca pe o
alegere a lui, nu ca pe o regula, si NU o generalizez: daca era intentionata, se pune pe element.
...
def curba(tri=False, inrush=False):
    """Litera curbei. `inrush` = sarcina cu varf la pornire (drivere LED, motoare) -> C."""
...
def breaker_type(tri=False, inrush=False, rccb_ma=None):
    ...
    bt = "MCB-%s-%s" % ("3P" if tri else "1P", curba(tri, inrush))
...
def eticheta(amp, tri=False, inrush=False, tip=None, este_tablou=False, din_general=False,
    ...
    out = "%s %s %dA %s %s" % (prefix, poli, int(amp or 0), curba(tri, inrush),
...
def normalizeaza(protectie, tri=None, inrush=False, tip=None, este_tablou=False,
    ...
    return " ".join(cap + [curba(tri, inrush), _ka_txt(icn_ka(tip, este_tablou, din_general))]
                    + coada)
